...
function setup() {
    ...
    filtered.loadPixels();
    // let stride = 1;
    for (let x = 1; x < dim - 1; x+= stride) {
        for (let y = 1; y < dim - 1; y+=stride) {
...
function pooling(img, x, y) {
    ...
    for(let i = 0; i < 2; i++) {
        for (let j = 0; j < 2; j++) {
            ...
            brightG = max(brightR, g);
            brightB = max(brightR, b);

        }
    }


    return{r: brightR ,g: brightG ,b: brightB};
}
